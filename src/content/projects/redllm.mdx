...
featured: true
badges: ["ai/ml"]
summary: "<strong>Enterprise-grade Red Teaming</strong> framework for LLM vulnerability assessment. System implements adaptive multi-round attack generation, ε-greedy bandit allocation, ensemble judging, and comprehensive jailbreak taxonomy.<ul><li>Adaptive AI Attack Generation (DSPy + ε-greedy)</li><li>Ensemble Judge — majority voting + confidence-based retries across multiple judge models</li><li>Multi-provider LLM Orchestration (LiteLLM)</li><li>Dual Output + Web UI Investigation (CSV + JSONL / details)</li></ul>"
artifacts:
  diagram: "/artifacts/redllm/results-matrix.png"
  readme: "https://github.com/kiwaku/RedLLM"
images:
  - "/artifacts/redllm/dashboard.png"
  - "/artifacts/redllm/results-matrix.png"
related: []
---
...
- **V2 pipeline (active):**
  - Generator → Validator → Executor → Ensemble Judge → Reporter
...
<div class="diagram-section">
  <a href="/artifacts/redllm/results-matrix.png" class="lightbox">
    <img src="/artifacts/redllm/results-matrix.png" alt="RedLLM Results Matrix — Demo Vulnerability Assessment against gpt-3.5-turbo and claude-3-haiku, judged by gpt-4. 9 variants across encoding, role-playing, emotional, technical, and misdirection strategies." />
  </a>
</div>

This results matrix is the end-product of a RedLLM run against two target models (gpt-3.5-turbo, claude-3-haiku) judged by gpt-4. Every variant is captured with its strategy, score, pass/fail outcome, and a response preview — allowing rapid identification of which attack families land and which the model deflects. The Output Inspector panel (right) expands any row to the full prompt and untruncated judge decision.
RedLLM preserves a complete audit trail (configuration → prompt lineage → judge decisions) so findings are reproducible and translate directly into remediation work.

<div class="diagram-section" style="margin-top:1rem;">
  <a href="/artifacts/redllm/dashboard.png" class="lightbox">
    <img src="/artifacts/redllm/dashboard.png" alt="RedLLM Dashboard — run history, status indicators, and quick-start workflow" />
  </a>
</div>
...
- **V2 pipeline is the sole active implementation:** clean separation of concerns (Generator → Validator → Executor → Ensemble Judge → Reporter).
...
- **Ensemble judge:** multiple judge models with majority voting and confidence-based retries; per-variant scoring + classification feeds both reporting and adaptive allocation. Single-model path retained when only one judge is supplied.
...
<div class="diagram-section" style="margin-bottom:1rem;">
  <video
    src="https://pub-ebd925cfbd3147d4a5c8e9c1db610d77.r2.dev/redllm/redLLM_demo1.mp4"
    controls
    muted
    preload="metadata"
    style="width:100%;max-width:100%;border:1px solid var(--line);"
  >
    Video: RedLLM Web UI demo — run configuration and results overview.
  </video>
</div>

<div class="diagram-section" style="margin-bottom:1rem;">
  <video
    src="https://pub-ebd925cfbd3147d4a5c8e9c1db610d77.r2.dev/redllm/redLLM_demo2.mp4"
    controls
    muted
    preload="metadata"
    style="width:100%;max-width:100%;border:1px solid var(--line);"
  >
    Video: RedLLM Web UI — importing results, filtering by strategy, and inspecting judge decisions.
  </video>
</div>
