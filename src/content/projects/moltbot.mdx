...
Moltbot was shelved in February 2026 in favour of [Qut](/projects/qut) — a sovereign AI coding lab built on the same [Hephaestus](/projects/hephaestus) hardware but without any third-party agent runtime.

The decision came down to supply-chain trust. OpenClaw's skill ecosystem had become an active attack surface: Koi Security's January 2026 disclosure found 341+ malicious skills in ClawHub — backdoors, credential exfiltrators, and prompt injection payloads packaged as productivity tools. The skill trust model was opt-out by default, meaning newly installed skills were executable unless explicitly denied. Separately, 1,009+ OpenClaw gateways were found exposed on Shodan with no authentication, a consequence of "auth mode none" being the default until a recent breaking change.

Auditing tools exist (skillvet, Cisco Skill Scanner), but they're reactive — always one step behind the next malicious upload. Hardening OpenClaw to the level required for personal infrastructure felt like building a fortress on someone else's foundation.

[Qut](/projects/qut) sidesteps the problem entirely: no skill marketplace, no gateway, no third-party runtime. Just a local model (Qwen3-Coder-30B on a Tesla P40) served over an air-gapped network, talked to by Aider and Claude Code. The attack surface is SSH and your own code — which is exactly what you'd have without any AI at all.
