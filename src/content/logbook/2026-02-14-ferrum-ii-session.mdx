...
| | Kernel 5.15 (stock) | Kernel 6.8 (HWE) |
|---|---|---|
| nvidia-peermem (inbox) | Loads, but `PCI_P2PDMA` disabled → EFAULT | `PCI_P2PDMA=y`, but peermem won't load (no IB peer mem API) |
| MLNX_OFED 4.9 | Might work (uncertain 5.15 support) | Won't support 6.8 |
| MLNX_OFED 5.x+ | Dropped ConnectX-3 entirely | Dropped ConnectX-3 entirely |
...
| Metric | GPU zero-copy | CPU baseline | Delta |
|---|---|---|---|
| Throughput | 2.69M pps | 2.80M pps | ~equal |
| Latency avg | **73 µs** | 3,104 µs | **42× lower** |
| Latency p99 | **116 µs** | 9,025 µs | **78× lower** |
| cudaMemcpy | **0 µs** | 1,061 µs/cycle | eliminated |
| CPU utilization | 12.3% | 11.0% | similar |
