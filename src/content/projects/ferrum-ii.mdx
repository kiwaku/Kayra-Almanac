...
{/* TODO: Fig — Full system architecture diagram (user will create as Mermaid/image) */}
...
{/* TODO: Fig — Ring buffer architecture diagram (user will create) */}
...
|---|---|---|
| CPU | 2× Intel Xeon E5-2683 v3 | 14 cores each, 2.0 GHz, 56 threads total |
| RAM | DDR4 | 64 GB |
| GPU | NVIDIA Tesla P40 | 24 GB VRAM, PCIe 3.0 ×16, Pascal GP102 |
| NIC (receiver) | Mellanox ConnectX-3 Pro | Dual QSFP+, 40 Gbps, PCIe 3.0 ×8 |
| NIC (sender) | Mellanox ConnectX-3 Pro | Dual QSFP+, 40 Gbps, PCIe 2.0 ×4 (adapter) |
| Cable | QSFP+ DAC | 40 Gbps passive copper |
...
| Component | Version |
|---|---|
| OS | Ubuntu 20.04.6 LTS (kernel 5.4.0-216) |
| RDMA stack | MLNX_OFED 4.9-7.1.0.0 |
| GPU driver | NVIDIA 550.163.01 |
| CUDA toolkit | 12.4 |
| GPUDirect | nvidia-peermem with `peerdirect_support=1` |
...
| Attack | Response | Size | Construction |
|---|---|---|---|
| SYN flood | TCP RST+ACK | 54 bytes | Swaps MAC/IP/port from attacker flow, sets RST+ACK flags |
| Port scan | Deceptive SYN-ACK | 54 bytes | Mimics open port (window=65535), deterministic ISN from flow hash |
| ICMP flood | ICMP Dest Unreachable | 70 bytes | Type 3/Code 1, embeds original IP header |
...
| Metric | GPU Zero-Copy | CPU Baseline | Improvement |
|---|---|---|---|
| Throughput | 2.62M pps (31.8 Gbps) | 3.14M pps (38.1 Gbps) | CPU slightly higher* |
| Latency (avg) | **82 µs** | 4,128 µs | **50× lower** |
| Latency (p99) | **119 µs** | 5,889 µs | **49× lower** |
| Latency (max) | 2,345 µs | 5,913 µs | 2.5× lower |
| CPU utilization | 12.5% | 10.4% | Similar |
| cudaMemcpy | **0 µs** | 2,316 µs/cycle | **Eliminated** |
| Valid packets | 100% | 100% | Identical |
| Detection accuracy | Correct | Correct | Same model |
...
|---|---|---|
| Parse + Flow (no model) | 2.76M pps | 79 µs |
| Parse + Flow + Classify | 2.62M pps | 82 µs |
| Parse + Flow + Classify + Respond | 2.72M pps | ~85 µs |
...
| Attack | Responses | Breakdown | All Checksums Valid | Detection-to-Response |
|---|---|---|---|---|
| SYN flood | 22 | 1 RST + 17 SYN-ACK + 4 ICMP | Yes | 146 µs |
| Port scan | 65,146 | ~65K SYN-ACK + ~150 ICMP | Yes | 282–977 µs |
| ICMP flood | 23 | 18 SYN-ACK + 5 ICMP | Yes | ~200 µs |
| Benign | 269* | 269 SYN-ACK (false positive) | Yes | — |
...
|---|---|---|---|---|
| Benign | 1.00 | 0.99 | 1.00 | 2,000 |
| SYN Flood | 1.00 | 1.00 | 1.00 | 2,000 |
| Port Scan | 0.99 | 1.00 | 1.00 | 2,000 |
| ICMP Flood | 1.00 | 1.00 | 1.00 | 2,000 |
...
| Test | Target Memory | Result |
|---|---|---|
| `ib_write_bw` (64 KB, RC QP) | System RAM | 12.31 Gbps |
| `ib_write_bw` (64 KB, RC QP) | GPU VRAM | 10.9 Gbps |
| `ib_read_lat` (2B, RC QP) | System RAM | 2.18 µs |
...
| File | Lines | Purpose |
|---|---|---|
| `gpu_ingestion_engine.cu` | 810 | Main receiver: VRAM ring buffer, CUDA pipeline, RDMA response path |
| `pkt_sender.c` | 593 | Sender: 6 traffic generator modes, pcap replay, response monitor |
| `cpu_baseline.cu` | 537 | CPU-mediated baseline for benchmark comparison |
| `flow_tracker.cuh` | 201 | GPU hash table: 65K entries, FNV-1a, atomic aggregation |
| `response_engine.cuh` | 272 | GPU packet crafting: checksums, RST/SYN-ACK/ICMP builders |
| `classifier.cuh` | 189 | CUDA MLP: forward pass per flow, binary weight loader |
| `rdma_common.h` | 160 | RDMA boilerplate: QP setup, transitions, TCP exchange |
| `common.h` | 118 | Shared structs and constants |
| `train_model.py` | 177 | Offline MLP training and weight export |
| **Total** | **~3,100** | |
